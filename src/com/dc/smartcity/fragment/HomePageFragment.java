...
public class HomePageFragment extends BaseFragment {
	...
	protected void updatePage(String result) {
		...
		// 加载按钮
		if (null != home.recommendServiceList && home.recommendServiceList.size() > 0) {
	 ...
	 * @param sceneColumnList
	 */
	private void modifyScenceItems(List<RelativeLayout> modules, List<ScenceItem> sceneColumnList) {
		for (int i = 0; i < modules.size(); i++) {
			...
			ImageLoader.getInstance().displayImage(sitem.getColumnPicUrl(), ivItem);
	...
	private void initGridView(final List<DCMenuItem> list) {
		...
		gridview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				DCMenuItem one = list.get(position);
				if (TextUtils.isEmpty(one.serviceUrl)) {

					startActivity(new Intent(getActivity(), ServiceMarketActivity.class));
				}
				else {
					if ("1".equals(one.level)) {
						if (!Utils.isLogon()) {
	...
	private void toCordoVaWeb(DCMenuItem one) {
		Intent intent = new Intent(getActivity(), CordovaWebwiewActivity.class);
		intent.putExtra(BundleKeys.WEBVIEW_LOADURL, one.serviceUrl);
		intent.putExtra(BundleKeys.WEBVIEW_TITLE, one.serviceName);
		startActivity(intent);
	}
