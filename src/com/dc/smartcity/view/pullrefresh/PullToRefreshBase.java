...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			if (startTime == -1) {
				...
				if (mOnPullDistanceChangedListener != null) {
					mOnPullDistanceChangedListener.onDistanceChanged(currentY);
				}
	...
	public static final int MODE_NONE = 0x0;//不显示时使用
	public static final int MODE_AUTO_REFRESH = 0x4;//自动刷新
	...
	private int mLimitPullDownHeight = 0;
	...
	public void setReleaseLabel(String releaseLabel) {
		if (null != getHeaderLayout()) {
			getHeaderLayout().setReleaseLabel(releaseLabel);
		}
		if (null != getFooterLayout()) {
			getFooterLayout().setReleaseLabel(releaseLabel);
		}
	}
	...
	public void setPullLabel(String pullLabel) {
		...
		if (null != getFooterLayout()) {
			getFooterLayout().setPullLabel(pullLabel);
		}
	}
	...
	public void setRefreshingLabel(String refreshingLabel) {
		if (null != getHeaderLayout()) {
			getHeaderLayout().setRefreshingLabel(refreshingLabel);
		}
		if (null != getFooterLayout()) {
			getFooterLayout().setRefreshingLabel(refreshingLabel);
		}
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_POINTER_DOWN:
				anothorMotionY = event.getY(event.getActionIndex());
				if (anothorMotionY != 0) {
					lastMotionY = initialMotionY = anothorMotionY;
				}
				break;
			...
			case MotionEvent.ACTION_MOVE: {
				if (isReadyForPull()) {
					...
					if (yDiff > touchSlop && yDiff > xDiff) {
						if ((mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH || mode == (MODE_PULL_DOWN_TO_REFRESH | MODE_AUTO_REFRESH)) && dy >= 0.0001f
								&& isReadyForPullDown()) {
							lastMotionY = y;
							isBeingDragged = true;
	...
	private void init(Context context, AttributeSet attrs) {
		...
		String StrPullToUpRefresh = context.getString(R.string.pull_to_refresh_pull_up_label);
		...
		String StrPullToDownRefresh = context.getString(R.string.pull_to_refresh_pull_down_label);
		String StrReleaseToRefresh = context.getString(R.string.pull_to_refresh_release_label);

		// Add Loading Views
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			setHeaderLayout(new LoadingLayout(context, MODE_PULL_DOWN_TO_REFRESH, StrReleaseToRefresh, StrPullToDownRefresh,
					refreshingLabel));
			addView(getHeaderLayout(), 0, new LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
			measureView(getHeaderLayout());
			headerHeight = getHeaderLayout().getMeasuredHeight();
		}
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			setFooterLayout(new LoadingLayout(context, MODE_PULL_UP_TO_REFRESH, releaseLabel, StrPullToUpRefresh, refreshingLabel));
			addView(getFooterLayout(), new LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
			measureView(getFooterLayout());
			headerHeight = getFooterLayout().getMeasuredHeight();
		}

		// Styleables from XML
		if (a.hasValue(R.styleable.PullToRefresh_pullheaderTextColor)) {
			final int color = a.getColor(R.styleable.PullToRefresh_pullheaderTextColor, Color.BLACK);
			if (null != getHeaderLayout()) {
		...
		if (a.hasValue(R.styleable.PullToRefresh_pullheaderBackground)) {
			this.setBackgroundResource(a.getResourceId(R.styleable.PullToRefresh_pullheaderBackground, Color.WHITE));
		}
	...
	private boolean pullEvent() {
		...
		int newHeight = Math.round(oldHeight - (lastMotionY - initialMotionY) / FRICTION);
		if (currentMode == MODE_PULL_UP_TO_REFRESH) {
			newHeight = Math.max(newHeight, 0);
		} else {
			newHeight = Math.min(newHeight, 0);

			if (mLimitPullDownHeight != 0) {
				newHeight = Math.max(newHeight, mLimitPullDownHeight);
			}
		}

		if (mOnPullDistanceChangedListener != null) {
			mOnPullDistanceChangedListener.onDistanceChanged(newHeight);
		}
		...
		if (newHeight != 0) {
			...
			switch (currentMode) {
				case MODE_PULL_UP_TO_REFRESH:
					getFooterLayout().refreshProgressBar(headerHeight, tempHeight);
					break;
				case MODE_PULL_DOWN_TO_REFRESH:
					getHeaderLayout().refreshProgressBar(headerHeight, tempHeight);
					break;
			}
			if (state == PULL_TO_REFRESH && headerHeight < tempHeight) {
			...
			} else if (state == RELEASE_TO_REFRESH && headerHeight >= tempHeight) {
				state = PULL_TO_REFRESH;
	...
	private boolean isReadyForPull() {
		switch (mode) {
			case MODE_PULL_DOWN_TO_REFRESH | MODE_AUTO_REFRESH:
			case MODE_PULL_DOWN_TO_REFRESH:
	...
	public void setLimitPullDownHeight(int limitPullDownHeight) {
		mLimitPullDownHeight = limitPullDownHeight;
	}
	...
	public static interface OnRefreshListener {
		/**
		 * @param curMode 当前刷新模式(返回值参考mode变量)
		 * @return 用于底部自动加载：
		 * true，显示底部"正在加载"；
		 * false，不显示
		 */
	...
	public void setOnPullDistanceChangedListener(OnPullDistanceChangedListener listener) {
		mOnPullDistanceChangedListener = listener;
	}
